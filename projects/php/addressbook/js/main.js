...
$(document).ready(function()
{
     ...
     */
    $.post('controllers/logs_controller.php', function(data) {
        limit = data;
    });
    ...
    $('#customer-list').on('click, change', function() {
        if ($('#customer-list option:selected').length) {
            ...
            if (!hasCustomer) {
                $.post('controllers/addressbook_controller.php', {customer_seq: customerSeq}, function(customer) {
    ...
    $('.display-all').click(function() {
        ...
        $('.left-pager').fadeOut();
        $.post('controllers/logs_controller.php', {customer: customerSeq}, function(data)
        {
            $('.logs tbody').empty();
            if (data.length < 1) {
    ...
    $('.left-pager').click(function() {
        if (offset >= 10) {
            ...
            $.post('controllers/logs_controller.php', {customer: customerSeq, offset: offset}, function(data)
            {
                var rows = "";
                $.each(data, function(j, event) {
...
function setCustomer(customer)
{
    ...
    $.post('controllers/logs_controller.php', {count: customer.seq}, function(data) {
        limit = data;
        if(limit > 10){
            $('.right-pager').fadeIn();
        }else{
    ...
    $.post('controllers/logs_controller.php', {customer: customer.seq}, function(data) {
        var rows = "";
        $.each(data, function(j, event) {
            rows += '<tr><td>' + event.date_time + "</td><td>" + event.action + '</td><td>' + event.class_seq + '</tr>';
        });
