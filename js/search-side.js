...
$(document).ready(function() {
	(function(){ // protect variables and functions from external interference
		...
		$.getJSON('/db/projects.json', function(data) {
			$(".side-results").empty(); // ensure div is empty first
			$(".side-results").append("All projects from most recent.");
			projects = data.projects; // add projects to reusable object
			/* load each project onto the page */
			var group = "";
			$.each(projects, function(i, project){	
				var p = (i % 5 === 0? "<hr />": "") + "<div class='side-result'><a href='"
							+ project.url + "'><img src='" + project.img + "' alt='"
							+ project.name + "'></a><div><a href='"	+ project.url
							+ "'><h5>" + project.name + "</h5></a></div><p>" + project.type + "</p></div><hr />";
				group += p;
				if((i+1) % 5 === 0){
			...
			pagiCurrent = 0;
			$(".side-results").append(pagiArray[pagiCurrent]);
			$(".pagination").empty();
			var x = 0;
			if(pagiCurrent > 2){
			...
			num = num > -3 && num < 3? num + x: -2 + x;
			for(var i = 0; i < 7 && i < pagiArray.length; i++){
				if(num === 1){
		...
		function pagination(){
			$(".side-results").html(pagiArray[pagiCurrent]);
			$(".pagination").empty();
			var x = 0;
			if(pagiCurrent > 2){
				x = pagiArray.length - pagiCurrent;
		...
		function doSearch(term, type){
			...
			$(".side-results").append(results.length + " Results for <strong>'" + term + "'</strong>"
			...
			$.each(results, function(i, result){
				if(result.value > 0){ // only display qualifying results
					...
					var p = (count % 5 === 0? "<hr />": "") + "<p><strong>Match Score "
							+ result.value + "</strong></p><p>"
							+ terms + "</p><div class='side-result'><a href='"
							+ project.url + "'><img src='" + project.img + "' alt='"
							+ project.name + "'></a><div><a href='"	+ project.url
							+ "'><h5>" + project.name + "</h5></a></div><p>" + project.type + "</p></div><hr />";
					group += p;
			...
			$(".side-results").append(pagiArray[0]);
			pagiCurrent = 0;
		}
